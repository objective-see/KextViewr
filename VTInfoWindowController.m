...
-(id)initWithItem:(Kext*)selectedKext
{
    self = [super init];
    if(nil != self)
    {
...
-(void)configure
{
    ...
    //get status
    if(nil != self.kext.vtInfo[VT_RESULTS_URL])
    {
        //known
        isKnown = YES;
    }
    ...
    if(YES == isKnown)
    {
        ...
        if(0 != [self.kext.vtInfo[VT_RESULTS_POSITIVES] unsignedIntegerValue])
        {
            //red
        ...
        vtDetectionRatio = [NSString stringWithFormat:@"%lu/%lu", (unsigned long)[self.kext.vtInfo[VT_RESULTS_POSITIVES] unsignedIntegerValue], (unsigned long)[self.kext.vtInfo[VT_RESULTS_TOTAL] unsignedIntegerValue]];
        
        //set name
        [self.fileName setStringValue:self.kext.name];
        ...
        makeTextViewHyperlink(self.analysisURL, [NSURL URLWithString:self.kext.vtInfo[VT_RESULTS_URL]]);
    ...
    else
    {
        ...
        [self.unknownFile setStringValue:[NSString stringWithFormat:@"no results found for '%@'", self.kext.name]];
...
-(IBAction)vtButtonHandler:(id)sender
{
    ...
    {
        //set status msg
        [self.statusMsg setStringValue:[NSString stringWithFormat:@"submitting re-scan request for %@", self.kext.name]];
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            if(nil != result)
            {
                ...
                //remove kext's current vt results
                self.kext.vtInfo = nil;
                ...
                if(nil != scanID)
                {
                    ...
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 60 * NSEC_PER_SEC), dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                        [vtObj getInfoForItem:self.kext scanID:scanID];
                    });
                ...
                [((AppDelegate*)[[NSApplication sharedApplication] delegate]) reloadRow:self.kext];
    ...
    {
        //set status msg
        [self.statusMsg setStringValue:[NSString stringWithFormat:@"submitting %@", self.kext.name]];
