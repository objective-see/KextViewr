...
-(void)enumKexts
{
    ...
    // already enumerating, so bail
    if(YES != self.refreshButton.enabled)
    {
        return;
    }
    ...
    //fade-out overlay
    // start kext enumeration
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        
        //hide overlay
        [self.kextTableController hideOverlay];
...
-(IBAction)refreshKexts:(id)sender
{
    ...
    //remove all kexts
    [self.kextEnumerator.kexts removeAllObjects];
    
    //refresh
    [self.kextTableController refresh];
    ...
    //show overlay
    [self.kextTableController showOverlay];
